  ...
  const handleProcessImages = async () => {
    ...
    try {
      for (let i = 0; i < images.length; i++) {
        ...
        // 2. Remove Exif if enabled
        let targetBase64 = base64Data;
        if (clearOriginalExif) {
          try {
        ...
        // 3. Compress/Resize on canvas if enabled OR if file format is PNG/WebP/GIF (since Exif requires JPEG)
        const isJpeg = imgFile.file.type === 'image/jpeg' || imgFile.file.type === 'image/jpg';
        if (compressImage || !isJpeg) {
        ...
        // 5. Insert EXIF back into image
        const outputBase64 = piexif.insert(exifBytes, targetBase64);
