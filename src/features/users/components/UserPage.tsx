...
export default function UserPage() {
	...
	const { users, selectedUser, loading, error, pagination } = useAppSelector((state) => state.users);
	...
	useEffect(() => {
		dispatch(fetchUsers({ page: pagination.page, limit: pagination.limit, search: debouncedSearch }));
	}, [dispatch, pagination.page, pagination.limit, debouncedSearch]);

	const handlePageChange = (newPage: number) => {
		dispatch(fetchUsers({ page: newPage + 1, limit: pagination.limit, search: debouncedSearch }));
	};

	const handleRowsPerPageChange = (newLimit: number) => {
		dispatch(fetchUsers({ page: 1, limit: newLimit, search: debouncedSearch }));
	};
	...
	const handleStatusChange = async (userToUpdate: typeof selectedUser, newStatus: string) => {
		if (userToUpdate) {
			...
			await dispatch(updateUser({ id: userToUpdate.id, data }));
		}
	};
