      ...
      <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
            ...
            {canReassign && (
              <Button variant="outlined" onClick={() => { setReassignUser(null); setReassignNote(''); setReassignOpen(true); }}>
                Chuyển người
              </Button>
            )}
      ...
      <Dialog open={reassignOpen} onClose={() => setReassignOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
          <Autocomplete
            ...
            renderInput={(params) => <TextField {...params} label="Người xử lý mới" size="small" />}
          />
