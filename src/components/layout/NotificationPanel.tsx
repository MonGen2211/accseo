...
const TAB_LABELS = ['Tất cả', 'Yêu cầu', 'Bài viết', 'Thay đổi', 'Đồng bộ'];
type TabFilter = 'all' | 'request' | 'article' | 'change' | 'sync';
const TAB_FILTERS: TabFilter[] = ['all', 'request', 'article', 'change', 'sync'];

function filterByTab(items: AppNotification[], filter: TabFilter): AppNotification[] {
  if (filter === 'all') return items;
  if (filter === 'request') return items.filter((n) => REQUEST_TYPES.includes(n.type));
...
export default function NotificationPanel() {
  ...
  const filtered = filterByTab(items, TAB_FILTERS[tab]);
    ...
    <>
      ...
      <Popover
        ...
        slotProps={{
          paper: {
            sx: {
              width: 380, maxHeight: 560, borderRadius: 3,
